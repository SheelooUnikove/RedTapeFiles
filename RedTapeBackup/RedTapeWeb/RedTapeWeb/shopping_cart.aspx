           ...
           <asp:Repeater ID="rpt_AddtocartList" runat="server">
			...
			<li class="shpng_cart" id="<%#Eval("viewId") %>">
				<ul>
					...
					<li onclick="openproductpage();"><%#Eval("productName") %></li>				
                    <li><input id="pro<%#Eval("viewId") %>" type="number" class="qty" min="1" max="10" value="<%#Eval("qty") %>"  onchange="updateproqty(<%#Eval("viewId") %>,<%#Eval("salePrice") %>)"/></li> 
					<li><%#Eval("salePrice") %></li>                                      
					<li id="sub<%#Eval("viewId") %>" class="sub"><%#Eval("subTotal") %></li>
        ...
        function deleteFromCartList(viewid, productId, attrId, Amount) {
            ...
            function success()
            {
                ...
                if (count == 0) {
                    $('#cartdiv').hide();
                    $('.cart_plc_order').hide();
                    $('#msg').html("There are no items in this cart.");
                }
            }
            function fail(error)
            {
